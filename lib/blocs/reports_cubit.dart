...
class ReportsCubit extends Cubit<ReportsState> {
  ...
  Future<void> fetchDailyTransactions({String? date}) async {
    emit(ReportsLoading());
    try {
      final String queryDate = date ?? DateTime.now().toIso8601String().split('T')[0];
  ...
  Future<void> fetchMonthlySales({String? month}) async {
    ...
    try {
      ...
      final String queryMonth = month ?? "${now.year}-${now.month.toString().padLeft(2, '0')}";
  ...
  Future<String> exportMonthlyReport(String yyyyMM) async {
    ...
    if (parts.length != 2) throw Exception('Invalid month format. Expected YYYY-MM');
    final year = parts[0];
    final month = parts[1];
